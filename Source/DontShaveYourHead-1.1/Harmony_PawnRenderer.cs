...
namespace DontShaveYourHead
{
	...
	{
		const float YOffset_PostHead = 0.03409091f;
		const float YOffset_OnHead = 0.03030303f;
		...
		public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
		{
			...
			foreach (var code in instructions)
			{
				...
				else if (writing && code.opcode == OpCodes.Ldloc_S && code.operand is LocalBuilder b2 && b2.LocalIndex == 11)
				{
